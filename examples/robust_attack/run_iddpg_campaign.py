#!/usr/bin/env python3
"""INDEPENDENT DDPG (IDDPG) attack campaign across the three MuJoCo maps (one script).

Independent DDPG attacker: the off-policy, NON-cooperative baseline. Unlike
MADDPG (a single shared centralized Q critic over the global share_obs), IDDPG
gives every attacker its OWN decentralized critic Q_i(o_i, a_i) trained purely on
that agent's local observation and its own action, and each actor maximizes only
its own Q_i. It is the off-policy analogue of IPPO (per-agent independent
critics) and the decentralized control the paper contrasts against the
centralized maddpg / stage_maddpg. Launched with HARL defaults:

    --algo iddpg --state_type EP --causal_critic_state False
...
wandb (matches the iddpg run so plots can pick them up):
    name  = iddpg_eps{tag}_seed{seed}
...
Deployment (one script, two machines) via the TARGETS env var:
  ...
  TARGETS=Ant-v4,Hopper-v4 CONC=3 nohup /venv/rl310/bin/python -u \
        ...
        > logs/campaign/iddpg_server.log 2>&1 &
  tail -f logs/campaign/STATUS_iddpg.txt
  ...
  TARGETS=HalfCheetah-v4 CONC=2 PYTHONPATH=$PWD nohup \
        ...
        examples/robust_attack/run_iddpg_campaign.py \
        > logs/campaign/iddpg_hc_local.log 2>&1 &
...
STATUS = os.path.join(LOGDIR, "STATUS_iddpg.txt")
...
def attack_ckpt(env_full, expname, seed):
    pats = glob.glob(
        f"{REPO}/results/robust_attack/{env_full}/iddpg/{expname}/seed-{seed:05d}-*/models/actor_agent0.pt"
    )
    return bool(pats)


def attack_cmd(env_full, eps_tag, eps_val, seed, vpath):
    expname = f"attack_iddpg_{short(env_full)}_eps{eps_tag}_seed{seed}"
    cmd = [
        PY, "-u", TRAIN,
        "--algo", "iddpg", "--env", "robust_attack", "--exp_name", expname,
        "--scenario", env_full,
        ...
        "--share_param", "False",
        # Independent DDPG: every attacker has its OWN decentralized continuous Q
        # critic Q_i(o_i, a_i) on its LOCAL obs + own action, and each actor
        # maximizes only its own Q_i. The global share_obs is deliberately
        ...
        "--wandb_name", f"iddpg_eps{eps_tag}_seed{seed}",
    ]
    return expname, cmd
...
def main():
    ...
    log_status("==== INDEPENDENT-DDPG CAMPAIGN START ====")
    ...
    run_pool(jobs, CONC)
    log_status("==== INDEPENDENT-DDPG CAMPAIGN COMPLETE ====")
    return 0
