#!/usr/bin/env python3
"""WITH-delta^o attack campaign for Hopper-v4 (SERVER).

Keeping delta^o was found to help (the obs-attacker advantage A^o = delta^o +
stage_lambda * A^a). This scheduler trains the Hopper victims (missing ones only)
...
This is the Hopper counterpart of run_ant_withdelta_campaign.py. wandb run names
use NO "nodelta" suffix (stage_mappo_eps*_seed*), so they land in the "stage"
column of the comparison plots and never collide with the nodelta runs.

Phases (strict order; concurrency = 3, --n_rollout_threads 8 each):
  Phase HOP-V : train any MISSING Hopper-v4 victims for seeds {1, 10, 100, 1000, 10000}.
  Phase HOP-A : per (seed, eps) launch {ippo, stage_mappo(delta^o)},
...
How to run on the server (use the env that actually has torch+mujoco+harl!):
  ...
  nohup /venv/rl310/bin/python -u \
        examples/robust_attack/run_hopper_withdelta_campaign.py \
        > logs/campaign/hopper_withdelta_campaign.log 2>&1 &
  tail -f logs/campaign/STATUS_hopper_withdelta.txt
"""
import glob
import os
import subprocess
...
STATUS = os.path.join(LOGDIR, "STATUS_hopper_withdelta.txt")
...
HOP_ENV = "Hopper-v4"
HOP_SEEDS = [1, 10, 100, 1000, 10000]
VICTIM_EXP = "victim6m"


def short(env_full):
    if env_full.startswith("Ant"):
        return "ant"
    if env_full.startswith("Hopper"):
        return "hopper"
    return "hc"
...
def main():
    ...
    log_status("==== HOPPER WITH-delta CAMPAIGN START ====")
    log_status(f"REPO={REPO}")
    log_status(f"PY={PY}")
    log_status(f"conc={CONC} threads={THREADS} steps={STEPS} stage_lambda={STAGE_LAMBDA} wandb={USE_WANDB}")

    # ---- Phase HOP-V: Hopper victims (train only the missing ones) ----
    jobs = victim_jobs(HOP_ENV, HOP_SEEDS)
    log_status(f"Phase HOP-V: {len(jobs)} victim run(s) to train")
    run_pool(jobs, CONC)
    log_status("==== Phase HOP-V (Hopper victims) COMPLETE ====")

    # ---- Phase HOP-A: Hopper ippo + stage_mappo(delta^o) attacks ----
    jobs = attack_jobs(HOP_ENV, HOP_SEEDS, ["ippo", "stage_mappo"])
    log_status(f"Phase HOP-A: {len(jobs)} attack run(s) to train")
    run_pool(jobs, CONC)
    log_status("==== Phase HOP-A (Hopper with-delta attacks) COMPLETE ====")

    log_status("==== HOPPER WITH-delta CAMPAIGN COMPLETE ====")
    return 0
