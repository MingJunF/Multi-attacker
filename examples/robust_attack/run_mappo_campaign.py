#!/usr/bin/env python3
"""VANILLA MAPPO attack campaign across the three MuJoCo maps (one script).

Standard HARL MAPPO attacker (NOT stage-aware): a single shared centralized
critic, vanilla simultaneous CTDE. The critic is fed the env's share_obs =
[victim obs s, victim.act(s + delta_o), stage one-hot]; actors unchanged. This
is the "standard CTDE / single shared baseline" the paper contrasts against
stage_mappo / stage_maddpg. Launched with HARL defaults:
...
wandb (matches the mappo local run so plots can pick them up):
    name  = mappo_eps{tag}_seed{seed}
...
Deployment (one script, two machines) via the TARGETS env var:
  ...
  TARGETS=Ant-v4,Hopper-v4 CONC=3 nohup /venv/rl310/bin/python -u \
        examples/robust_attack/run_mappo_campaign.py \
        > logs/campaign/mappo_server.log 2>&1 &
  tail -f logs/campaign/STATUS_mappo.txt
  ...
  TARGETS=HalfCheetah-v4 CONC=2 PYTHONPATH=$PWD nohup \
        ...
        examples/robust_attack/run_mappo_campaign.py \
        > logs/campaign/mappo_hc_local.log 2>&1 &
...
STATUS = os.path.join(LOGDIR, "STATUS_mappo.txt")
...
def attack_ckpt(env_full, expname, seed):
    pats = glob.glob(
        f"{REPO}/results/robust_attack/{env_full}/mappo/{expname}/seed-{seed:05d}-*/models/actor_agent0.pt"
    )
    return bool(pats)


def attack_cmd(env_full, eps_tag, eps_val, seed, vpath):
    expname = f"attack_mappo_{short(env_full)}_eps{eps_tag}_seed{seed}"
    cmd = [
        PY, "-u", TRAIN,
...
def main():
    ...
    log_status("==== VANILLA-MAPPO CAMPAIGN START ====")
    ...
    run_pool(jobs, CONC)
    log_status("==== VANILLA-MAPPO CAMPAIGN COMPLETE ====")
    return 0
