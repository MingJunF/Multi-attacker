    ...
    def get_env_fn(rank):
        def init_env():
            if env_name == "SatBench":
                ...
                env = LAGEnv(env_args)
            elif env_name == "robust_attack":
                from harl.envs.robust_attack.robust_attack_env import (
                    RobustAttackEnv,
                )

                env = RobustAttackEnv(env_args)
            elif env_name == "robust_victim":
                from harl.envs.robust_attack.victim_env import RobustVictimEnv

                env = RobustVictimEnv(env_args)
            else:
...
def get_num_agents(env, env_args, envs):
    ...
    if env == "SatBench":
        ...
        return envs.n_agents
    elif env == "robust_attack":
        return envs.n_agents
    elif env == "robust_victim":
        return envs.n_agents
