...
class MAPPO_Potential(OnPolicyBase):
    def __init__(self, args, obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        self.potential_weight = args.get("potential_weight", 0.2) # default 0.1
        self.exploration_bonus_weight = args.get("exploration_bonus_weight", 0.05) #default 0.02
        self.entropy_bonus_weight = args.get("entropy_bonus_weight", 0.05) #default 0.02
        self.entropy_low_thresh = args.get("entropy_low_thresh", 0.7) #default 0.6
        self.stagnation_penalty = args.get("stagnation_penalty", 0.2) #default 0.1
    ...
    def update(self, sample):
        ...
        if next_obs_batch.ndim == 3:
            next_obs_batch = next_obs_batch[:, 0, :]

        obs_batch_flat = torch.tensor(obs_batch.reshape(-1, obs_batch.shape[-1]), device=self.device, dtype=torch.float32)
        next_obs_batch_flat = torch.tensor(next_obs_batch.reshape(-1, next_obs_batch.shape[-1]), device=self.device, dtype=torch.float32)

        centralized_obs_batch = torch.cat([obs_batch_flat] * self.num_agents, dim=-1)
        centralized_next_obs_batch = torch.cat([next_obs_batch_flat] * self.num_agents, dim=-1)

        current_v, _ = self.critic.critic(centralized_obs_batch, rnn_states_batch, masks_batch)
        next_v, _ = self.critic.critic(centralized_next_obs_batch, rnn_states_batch, masks_batch)

        current_v = current_v.detach()
        next_v = next_v.detach()

        # === Potential Difference Bonus ===
        delta_v = next_v - current_v
        adv_targ = adv_targ + self.potential_weight * delta_v

        # === Potential-Based Stagnation Penalty ===
        stagnant_mask = (delta_v.abs() < self.potential_stagnation_eps).float()
        ...
        # === Nash Gain Exploration Bonus ===
        with torch.no_grad():
            epsilon = torch.randn_like(centralized_obs_batch) * 0.01
            q_deviated, _ = self.critic.critic(centralized_obs_batch + epsilon, rnn_states_batch, masks_batch)
            bonus = (q_deviated - current_v).clamp(min=0).mean()

        policy_loss = policy_action_loss - self.exploration_bonus_weight * bonus

        # === Entropy Regularization Bonus ===
        entropy_bonus = (dist_entropy < self.entropy_low_thresh).float().mean()
        policy_loss = policy_loss - self.entropy_bonus_weight * entropy_bonus
