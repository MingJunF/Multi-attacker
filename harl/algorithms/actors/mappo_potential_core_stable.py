...
class MAPPO_Potential(OnPolicyBase):
    def __init__(self, args, obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        self.core_bonus_weight = args.get("core_bonus_weight", 0.2)
    ...
    def update(self, sample):
        ...
        # Core-based formation bonus
        B = batch_size // agent_num
        obs_seq = obs_batch.view(B, agent_num, -1)
        coords = obs_seq[..., 2:4]  # x, y 坐标

        core_bonus = []
        for b in range(B):
            group_bonus = []
            for i in range(agent_num):
                dist = torch.norm(coords[b] - coords[b, i], dim=1)
                neighbors = (dist < 1.25) & (dist > 1e-3)
                formation_value = torch.exp(-dist[neighbors]).sum()
                if formation_value > 0:
                    bonus_i = formation_value / (neighbors.sum() + 1e-5)
                else:
                    bonus_i = torch.tensor(0.0, device=coords.device)
                group_bonus.append(bonus_i)
            group_bonus = torch.stack(group_bonus, dim=0)
            core_bonus.append(group_bonus)
        core_bonus = torch.stack(core_bonus, dim=0).view(-1, 1)
        core_bonus = core_bonus / (core_bonus.max() + 1e-5)
        adv_targ = adv_targ + self.core_bonus_weight * core_bonus.detach()
    ...
    def train(self, actor_buffer, advantages, state_type):
        """train for non-parameter-sharing MAPPO"""
    ...
    def share_param_train(self, actor_buffer, advantages, num_agents, state_type):
        """train for parameter-sharing MAPPO"""
        train_info = {"policy_loss": 0, "dist_entropy": 0, "actor_grad_norm": 0, "ratio": 0}
        if state_type == "EP":
            advantages_ori_list = []
