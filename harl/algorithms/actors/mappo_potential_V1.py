...
class MAPPO_Potential(OnPolicyBase):
    def __init__(self, args, obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        self.potential_weight = args.get("potential_weight", 0.3)  # default 0.3
        self.num_agents = num_agents
        self._exploration_bonus_window = None
        self._stagnation_count = None
        self._entropy_adaptor = 0.5
        self.coop_bonus_weight = args.get("coop_bonus_weight", 0.3)  # default 0.3
        self.spatial_bonus_weight = args.get("spatial_bonus_weight", 0.2)  # default 0.2
        self.local_exploration_weight = args.get("local_exploration_weight", 0.1)
        self.obc_weight = args.get("obc_weight", 0.1)
    ...
    def update(self, sample):
        ...
        assert obs_batch_flat.shape[1] % agent_num == 0, \
            f"Inconsistent shape: obs_flat={obs_batch_flat.shape[1]}, agent_num={agent_num}"
        ...
        # === Spatial Formation Bonus ===
        obs_seq = obs_batch_flat.view(batch_size, agent_num, local_dim)
        coords = obs_seq[..., :2]  # assume first 2 dims are x, y coords
        centroid = coords.mean(dim=1, keepdim=True)
        dispersion = ((coords - centroid)**2).sum(dim=2).mean(dim=1)  # [B]
        spatial_bonus = -dispersion.unsqueeze(1).expand_as(adv_targ)  # [B*N, 1] or [B*N]
