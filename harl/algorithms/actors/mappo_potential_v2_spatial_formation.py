...
# 通用局部 spatial bonus 模块
def general_spatial_bonus(coords, neighbor_radius=1.25, sigma=0.2):
    batch_size, agent_num, _ = coords.shape
    spatial_bonus = torch.zeros(batch_size, agent_num, device=coords.device)

    for i in range(agent_num):
        diff = coords - coords[:, i:i+1, :]
        dist_sq = (diff ** 2).sum(dim=-1)

        neighbor_mask = (dist_sq < neighbor_radius ** 2) & (dist_sq > 1e-8)
        effective_neighbors = neighbor_mask.float().sum(dim=1).clamp(min=1.0)

        bonus = torch.exp(-dist_sq / (2 * sigma ** 2)) * neighbor_mask.float()
        bonus = bonus.sum(dim=1) / effective_neighbors

        spatial_bonus[:, i] = bonus

    spatial_bonus = spatial_bonus.unsqueeze(-1).view(-1, 1)
...
class MAPPO_Potential(OnPolicyBase):
    def __init__(self, args, obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        self.spatial_bonus_weight = args.get("spatial_bonus_weight", 0.2)
    ...
    def update(self, sample):
        ...
        local_dim = obs_batch_flat.shape[1] // agent_num
        ...
        # === General Spatial Formation Bonus ===
        B = batch_size // agent_num
        obs_seq = obs_batch.view(B, agent_num, -1)
        coords = obs_seq[..., 2:4]

        spatial_bonus = general_spatial_bonus(coords, neighbor_radius=0.5, sigma=0.2)
        adv_targ = adv_targ + self.spatial_bonus_weight * spatial_bonus.detach()
