...
class MAPPO_Potential(OnPolicyBase):
    def __init__(self, args, obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        self.potential_weight = args.get("potential_weight", 0.3)  # default 0.5
        ...
        self.coop_bonus_weight = args.get("coop_bonus_weight", 0.1)
    ...
    def update(self, sample):
        ...
        # 构造中央化观测：将局部观测扁平化后复制 agent 数次拼接
        ...
        with torch.no_grad():
            ...
            potential_term = adaptive_weight * torch.tanh(delta_v_normalized).detach()

        # 潜在奖励 shaping
        adv_targ = adv_targ.detach() + potential_term.detach()
        ...
        obs_local_padded = torch.zeros(batch_size, centralized_obs.shape[1], device=self.device)
        obs_local_padded[:, :obs_local_dim] = obs_local

        if next_obs_local is not None:
            next_obs_local_padded = torch.zeros(batch_size, centralized_obs.shape[1], device=self.device)
        ...
        if self._exploration_bonus_window is None:
            self._exploration_bonus_window = torch.zeros_like(exploration_bonus_local)
        self._exploration_bonus_window = 0.9 * self._exploration_bonus_window + 0.1 * exploration_bonus_local

        adv_targ = adv_targ + self._exploration_bonus_window.detach()

        # ===== 新增：基于反事实遮蔽的差分奖励激励 =====
        # 对于每个 agent，将其局部观测置零（模拟其缺席），计算全局价值基线，再求差分奖励
        diff_rewards = []
        ...
        for i in range(agent_num):
            ...
            masked_obs[:, start:end] = 0  # 模拟 agent i 不参与
            ...
            diff_reward_i = current_v - baseline_i
            diff_rewards.append(diff_reward_i)
        # 取各 agent 差分奖励的平均值作为合作激励
        ...
        policy_loss = policy_loss - self.local_exploration_weight * self._exploration_bonus_window.mean().detach()
