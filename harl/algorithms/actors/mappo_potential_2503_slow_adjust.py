...
class MAPPO_Potential(OnPolicyBase):
    def __init__(self, args, obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        self.potential_weight = args.get("potential_weight", 0.05) # default 0.1
        self.exploration_bonus_weight = args.get("exploration_bonus_weight", 0.02) #default 0.02
        self.stagnation_penalty = args.get("stagnation_penalty", 0.05) #default 0.1
    ...
    def update(self, sample):
        ...
        adv_mean, adv_std = adv_targ.mean(), adv_targ.std(unbiased=False) + 1e-8
        adv_targ = (adv_targ - adv_mean) / adv_std

        if next_obs_batch.ndim == 3:
            next_obs_batch = next_obs_batch[:, 0, :]

        obs_batch_flat = torch.tensor(obs_batch.reshape(-1, obs_batch.shape[-1]), device=self.device)
        next_obs_batch_flat = torch.tensor(next_obs_batch.reshape(-1, next_obs_batch.shape[-1]), device=self.device)

        centralized_obs = torch.cat([obs_batch_flat] * self.num_agents, dim=-1)
        centralized_next_obs = torch.cat([next_obs_batch_flat] * self.num_agents, dim=-1)
        ...
        current_v, next_v = current_v.detach(), next_v.detach()

        # Potential-based advantage adjustment
        delta_v = (next_v - current_v) / (current_v.std() + 1e-5)
        adv_targ = adv_targ + self.potential_weight * torch.tanh(delta_v)
        ...
        if self.use_policy_active_masks:
            policy_loss = (-torch.min(surr1, surr2) * active_masks_batch).sum() / active_masks_batch.sum()
        else:
            policy_loss = -torch.min(surr1, surr2).mean()

        # Exploration bonus
        with torch.no_grad():
            epsilon = torch.randn_like(centralized_obs) * 0.01
        ...
        # Final loss (policy + entropy + exploration)
        loss = policy_loss - self.entropy_coef * dist_entropy - self.exploration_bonus_weight * bonus
        ...
        return loss, dist_entropy, actor_grad_norm, imp_weights
